...
// import styles from "./applicationLink.module.css";
...
//   const {
//     data: ApplicationLinkData,
//     error: applicationLinkError,
//     isLoading,
//     refetch,
//   } = useGetApplicationlinkdataQuery({});

//   useEffect(() => {
//     setApplicationLink(ApplicationLinkData?.data?.data);
//   }, [ApplicationLinkData]);

//   const handleDelete = (appLinkId) => {
//     setApplicationLink(
//       ApplicationLinkData.filter((data) => appLinkId.id !== data.id)
//     );
//     toast.dismiss();
...
//         {ApplicationLinkData?.data?.length == 0 || applicationLinkError ? (
//           <div className={styles.addwebinar_no_data_found}>
//             <p className={styles.addwebinar_no_data_found_fnt}>
//               No Data Found Please Add Application Link
//             </p>
//           </div>
//         ) : (
...
//               </thead>
//               <tbody>
//                 {ApplicationLinkData?.data?.map((applicationLink, index) => (
//                   <ApplicationTable
//                     key={index}
//                     index={index}
...
const ApplicationLink = () => {
  ...
  // RTK Queries
  const { data, isLoading, refetch } = useGetApplicationlinkdataQuery();
  const { data: stateDegreeData } = useGetApplicationlinkstatequeryQuery();
  const [addApplicationLink] = useAddapplicationLinkMutation();
  const [updateApplicationLink] = useUpdateapplicationLinkMutation();
  const [deleteApplicationLink] = useDeleteapplicationLinkMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editData) {
        ...
        await addApplicationLink(formData).unwrap();
        toast.success("Application link added successfully");
      }
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "This action will permanently delete the application link.",
      icon: "warning",
    ...
    try {
      await deleteApplicationLink(id).unwrap();
      ...
      toast.error(error?.data?.message || "Failed to delete application link");
    }
  };
    ...
    <div className="p-4 bg-light min-vh-100">
      ...
      <Modal show={show} onHide={handleClose} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {editData ? "Edit Application Link" : "Add Application Link"}
          </Modal.Title>
        </Modal.Header>
